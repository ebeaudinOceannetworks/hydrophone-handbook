...
*This page is about **how to get hydrophone data**. For what each file type contains, see [Data Products](8-data-products.qmd).*

# Which method to use?
There are threee ways to get the data: two of them inside ONC data portal [Oceans3.0](https://data.oceannetworks.ca), plus one using ONC API.
...
**Data Search** is useful for when you want to download more than a few files at a time, whereas the Hydrophone Viewer allows for a single file at a time.
Whether it is a few hours, or even 1 day of data from a particular location.
There are a lot more user options that can be set through that interface.

Use the **ONC API** when you need more than a handful of files, or a data product with specific settings.
...
From **Hydrophone Viewer**, a list of locations will appear on the left panel, under Hydrophone Data Search. 

Pick a location. Go down the tree until you can select a device.
Note that each hydrophone has a dedicated *device code*, which are often a combination of their manufacturer/supplier and a serial number (*ex: Ocean Sonics icListen HF Hydrophone 1251*).
...
Navigate to [Hydrophone Map](3-hydrophone-map.qmd) page to find past and current deployments.

![](../images/Hydrophone-Viewer-step-3-4.png){lightbox=true}

### 4. Choose a date
The date is in the following format: 01-Jan-2026.
...
Each spectrogram is a visual representation of the soundscape over a 5-min time window. You can click on a spectrogram and it will size up.
...
![](../images/Data-Search-step-1-2.png){lightbox=true}

### 3. Choose a location or a hydrophone

From **Data Search**, a list of locations will appear on the left panel, under Data Source Selection.
If you already know the device code (ex: Ocean Sonics icListen HF Hydrophone 1352), there is also the option to Sort by: Instruments by Category and jump to Hydrophone.

Pick a location, or a hydrophone.

![](../images/Data-Search-step-3-4.png){lightbox=true}

### 4. Go to data source

The window pops up showing all information related to the hydrophone(s) at that location.
Click on Select This Data Source.

### 5. Select the data product

This page is where to select the data products needed from this location.
Set a **Date From** and **Date To** in the upper left.
Use the visual green overlay to the right to know about data availability for this specific location.

Certain hydrophones (mostly Ocean Sonics) have *ancillary data*, such as acceleration, internal humidity, temperature, etc.
Most likely, the data you are interested in rather lies in the audio format or derived spectral data.
You can select a data product by simply checking the box underneath.
For each data product, several formats are available, such as pdf, .mat, .txt, etc.  

![](../images/Data-Search-step-5-6-7.png){lightbox=true}

### 6. Select the product options
Most data product orders can also be customized through the *Data Product Options* panel below.

### 7. Add to cart

![](../images/Data-Search-add-to-cart.png){lightbox=true}

When you are satisfied with your order, do not forget to **Add to Cart**.
This simply adds the product to the cart, which you can find in View Cart in the top right.

Go to Cart to trigger the creation of the order, which may take from seconds to minutes depending on the amount of data requested.
...
Visit the [Hydrophone Map](3-hydrophone-map.qmd) page to find location or device codes of past and current deployments.
...
<form id="api-builder" class="api-builder" autocomplete="off">
  <div class="api-row">
    <div class="api-row-label">
      ...
      <p class="api-hint">Find location or device codes on the Map page.</p>
    </div>
