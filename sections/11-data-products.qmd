...
Each data product (`dataProductCode`) has a set of data product options (`dpo_*`) that can be used to narrow down the request. Open a product tab below for its options.
...
Audio data is the raw recording of the acoustic environment. Use it when you need the time series itself (listening, custom processing, or publication-quality archives).

Older files were stored as `.wav`; ONC now archives lossless `.flac`. `.mp3` is smaller but lossy, and is not recommended for scientific work. Retired IOS arrays used HARD / 38 (`.hyd`) instead of AD.
MP3 discards frequency content to shrink the file. That is fine for a quick listen, not for calibrated analysis. If you need a different container, request AD as `.wav` or set `dpo_audioFormatConversion` rather than transcoding a lossy copy.

FLAC (Free Lossless Audio Codec) is the preferred archive because of:
...
<div class="dp-meta">
  ...
  <span class="dp-pill"><span class="dp-pill-k">Available Formats</span> .fft, .mat, .png, .pdf, .oct</span>
</div>
```

Spectral products turn audio files into frequency vs time data.
The Hydrophone Viewer presents has spectrograms for each 5-min of audio file for each devices.
This product is a useful way to monitor and research when you do not want to process terabytes of raw audio.

![5-min spectrogram of a boat passing by a hydrophone at Digby Island.](../images/ICLISTENHF1252_20260814T210509.000Z-spect.png)

**`.fft` — proprietary spectral data (Ocean Sonics only).** Broadband spectrogram files, roughly a raw version of the plot. They use a single broadband sensitivity (around −165 to −170 dB) rather than a frequency-dependent calibration curve, so they are preliminary and best for a functional check. Ocean Sonics *Lucy* can read them; they can also be opened in Python or MATLAB.

**`.mat` — processed spectral data.** 1-minute averaged calibrated spectral data which comes with the device-specific sensitivity curve. Contains a `SpectData` structure with frequency bins (Hz) and power spectral density.
...
Fourier transforms assume an infinite, stationary signal. A finite cut creates edge discontinuities and **spectral leakage** into neighbouring bins. Windowing (tapering) multiplies the segment by a shape that goes to zero at the edges; that multiplication is a convolution, so the spectrum is the product of the signal transform and the window transform.
...
HSPD is a statistical view of long-term ambient noise, originally proposed by [Merchant et al. (2013)](https://pubs.aip.org/asa/jasa/article/133/4/EL262/917238/Spectral-probability-density-as-a-tool-for-ambient). It counts how often a given dB level occurs in each frequency bin. Use it to see the persistent noise floor rather than a single 5-minute snapshot.
...
Auxiliary (ancillary) files are about the instrument, not the soundscape: internal humidity, temperature, battery, voltage, and related engineering scalars. Use them to diagnose leakage (humidity above about 50%), overheating, or a dying battery. Magnetic flux and acceleration can also indicate orientation and strong currents. These products are available mainly on Ocean Sonics icListen devices.

These data are only available for Ocean Sonics hydrophones.


![Example of auxiliary data for Ocean Sonics icListen HF Hydrophone 1252. Top to bottom: Board voltage, Internal Humidity, and Internal Temperature.](../images/aux-data.png)
