...
*This page describes each hydrophone data product. To learn how to download files, go to the [Data Access](12-data-access.qmd).*

# Overview

Hydrophone data files are generated in 5-minute intervals, categorized as either **raw sensor recordings** (direct sound recording) or **derived products** (post-processing of audio data).


| Data Type | Product Name | Code / ID | Extensions |
| :--- | :--- | :---: | :---: |
| **Raw** | Audio Data | `AD` (7) | `.flac`, `.wav`, `.mp3` |
| **Raw** | Array Data *(Retired)* | `HARD` (38) | `.hyd` |
| **Raw** | Acceleration Data | `HACC` (128) | `.acc` |
| **Raw** | Auxiliary Log File | `LF` (4) | `.txt` |
| **Derived** | Hydrophone Spectral Data | `HSD` (45, 146) | `.fft`, `.mat`, `.png`, `.pdf`, `.oct` |
| **Derived** | Spectral Probability Density | `HSPD` (51) | `.mat`, `.png`, `.pdf` |

: Hydrophone Data Products {tbl-colwidths="[15,40,15,30]"}
...
::: {.dp-visual}
![Amplitude and frequency of an acoustic signal.](../images/AD.png){lightbox=true}
:::
...
::: {.dp-visual}
![5-min spectrogram of a boat passing by a hydrophone at Digby Island.](../images/ICLISTENHF1252_20260814T210509.000Z-spect.png){lightbox=true}
:::
...
```{=html}
<div class="dp-meta">
  <span class="dp-pill"><span class="dp-pill-k">Data Product Code</span> HSD / 45, 146</span>
  <span class="dp-pill"><span class="dp-pill-k">Available Formats</span> .mat, .fft, .png, .pdf </span>
</div>
```
...
Choosing the right window requires a trade-off between resolution in time or frequency.
...
::: {.dp-visual}
![Hydrophone spectral probability density plot.](../images/ICLISTENHF6021_20250910T211503.000Z_20251010T000000.000Z-SPD.png){lightbox=true}
:::
...
For a band-pass filtered signal $x(t, f, \Delta f)$, the mean square acoustic pressure within bandwidth $\Delta f$ over observation period $T$ is given by:

$$
\Psi^2_x(f,\Delta f) = \lim_{T\rightarrow \infty} \frac{1}{T} \int_0^T x(t,f,\Delta f)^2 dt
$$

The single-sided Power Spectral Density (PSD), $G_x(f)$, defines the mean square value per bandwidth as $\Delta f$ approaches zero:
...
Consequently, acoustic power within a narrow band is approximated by $\Psi^2_x(f,\Delta f)\approx G_x(f)\Delta f$.

The Hydrophone Spectral Probability Density (HSPD) evaluates bins individual PSD into a statistical histogram.
The color on the plot represents a probability density (occupancy frequency), or the percentage of time the noise level at that frequency $f$ was measured at intensity level $L$.
:::
...
::: {.dp-visual}
![Example of auxiliary data for an Ocean Sonics icListen HF. Top to bottom: board voltage, internal humidity, and internal temperature.](../images/aux-data.png){lightbox=true}
:::
