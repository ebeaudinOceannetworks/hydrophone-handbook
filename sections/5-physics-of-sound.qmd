...
::: {.fig-full}
![A variety of sounds can be heard in the ocean. Credit: NOAA Fisheries.](../images/1500x1000-Passive-Acoustics-Soundscape-infographic-NEFSC.png){lightbox=true}
:::
...
![Sound is a longitudinal wave whose particle displacement is parallel to the direction of travel. Credit: Dan Russell, 2015.](../images/Lwave-Red-2.gif){lightbox=true}
...
![Bigger objects make low-frequency sounds, while smaller objects make high-frequency sounds. Credits: Aline Design for NOAA](../images/Amplitude-Frequency.png){lightbox=true}
...
## Bulk modulus
Resistance to compression, $B=\rho \,\partial P / \partial \rho$.
:::
...
![Typical seasonal sound speed profile in the Northeast Pacific, including waters off British Columbia.](../images/ssp_seasonal.png){lightbox=true}
...
![Model simulation of sound trapped in a surface duct. Credit: Lynch et al., 2018, Fig. 5.](../images/surface-duct.png){lightbox=true}
...
::: {.fig-full}
![Acoustic sound paths in the ocean. Credit: National Academy of Sciences.](../images/sound-propagation.png){lightbox=true}
:::
...
| **Refraction** | The ray bends toward slower sound speed as $T$, $S$, or pressure change ([Snell's law](15-acoustic-equations.qmd#snells-law)). | SOFAR and surface ducts; shadow zones. |
...
<!-- [Example of refracted ray paths. Credit: Kessel and Hollett, 2006.](../images/refraction.png){lightbox=true} -->

<!-- ![Constructive and destructive interference.](../images/interference.png){lightbox=true} -->

# Transmission loss

From a source (a whale) to a receiver (a hydrophone), the source level (SL), measured in decibels (dB), can drop significantly.
That drop is due to **transmission loss (TL)**, or propagation loss. The received sound level (RL) becomes:

$$
RL = SL - TL
$$

TL splits into geometric spreading (GS) and everything else (excess attenuation, EA):

$$
TL = GS + EA
$$

![Model output of transmission loss of an acoustic sound wave.](../images/SCM4_fig11.png){lightbox=true}
...
![Typical sound speed profile in the Arctic.](../images/arctic-sound-speed-profile.png){lightbox=true}
...
* behaviour: marine mammals can get lost, and reduce or no longer mate. 
...
Ray tracing is used at ONC to locate sources, estimate detection range of our hydrophones, plan new deployment sites, and avoid “deaf zones.”

There are multiple sound propagation models, each with different accuracies depending on the frequency of the emitting sound, and the complexity of the environment the sound is propagating into (e.g., rugged terrain, canyons, highly stratified waters).
Each model essentially *solves* the [wave equation](15-acoustic-equations.qmd#wave-equation) to compute the transmission loss along the propagation path.

**BELLHOP** (Michael B. Porter) is one of the many sound propagation models that traces beams at a chosen frequency.
The required parameters include the bathymetry along the path, a sound-speed profile (from $T$, $S$, $p$), and a bottom type (e.g. clay).

![Example of ray tracing (right) using BELLHOP with a specific sound speed profile (left).](../images/Bellhop-ray-tracing-right-box-for-a-standard-sound-speed-profile-left-box-indicating.png){lightbox=true}
